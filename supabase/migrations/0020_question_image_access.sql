-- A question image is part of the public question and remains available to
-- joined players throughout the question. Reveal-only images stay gated until
-- answer reveal.
...
begin
  ...
  if p_player_token is not null and exists (select 1 from public.session_players p where p.session_id = active_session.id and p.player_token_hash = public.token_hash(p_player_token)) then
    return active_session.state @> jsonb_build_object('question', jsonb_build_object('questionImageAssetId', p_asset_id::text))
      or active_session.state @> jsonb_build_object('question', jsonb_build_object('options', jsonb_build_array(jsonb_build_object('imageAssetId', p_asset_id::text))))
