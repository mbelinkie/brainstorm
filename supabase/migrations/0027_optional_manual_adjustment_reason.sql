-- A blank host-provided reason should remain blank in score-event exports and
-- presentation celebrations; it is not a substitute for a default label.

alter table public.score_events alter column reason drop not null;
alter table public.score_events drop constraint if exists score_events_reason_check;
alter table public.score_events add constraint score_events_reason_check check (reason is null or char_length(reason) between 1 and 160);
