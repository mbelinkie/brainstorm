-- Closest-number questions accept a single numeric guess. Every player tied
-- for the smallest absolute distance from `targetNumber` receives an equal
-- share of the question's points.
...
begin
  ...
  if active_question ->> 'type' = 'closest_number' and coalesce(active_question ->> 'targetNumber', '') ~ '^[+-]?([0-9]+([.][0-9]+)?|[.][0-9]+)$' then
    ...
    select min(abs((answer #>> '{}')::numeric - target_number)) into winning_distance
    from public.submissions
    where session_id = active_session.id
      and question_id = active_session.state ->> 'questionId'
      and answer #>> '{}' ~ '^[+-]?([0-9]+([.][0-9]+)?|[.][0-9]+)$';
    if winning_distance is not null then
      ...
      where session_id = active_session.id
        and question_id = active_session.state ->> 'questionId'
        and answer #>> '{}' ~ '^[+-]?([0-9]+([.][0-9]+)?|[.][0-9]+)$'
        and abs((answer #>> '{}')::numeric - target_number) = winning_distance;
  ...
  for answer_row in select * from public.submissions where session_id = active_session.id and question_id = active_session.state ->> 'questionId' loop
    ...
    if active_question ->> 'type' in ('single_choice', 'true_false', 'image_selection') then
      ...
      if shared_points is not null and answer_text ~ '^[+-]?([0-9]+([.][0-9]+)?|[.][0-9]+)$' and abs(answer_text::numeric - target_number) = winning_distance then
        awarded_points := shared_points;
        score_reason := case when winner_count = 1 then 'Closest number' else format('Closest number (tied %s ways)', winner_count) end;
      end if;
    end if;
    if awarded_points > 0 then insert into public.score_events (session_id, player_id, question_id, points, reason, created_by) values (active_session.id, answer_row.player_id, active_session.state ->> 'questionId', awarded_points, score_reason, 'system'); score_count := score_count + 1; end if;
  end loop;
