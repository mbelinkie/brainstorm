...
- Fixed the Presentation clip pool printing the raw text `${showingMatches ? "All items placed" : "Listen for each clip"}` on the shared screen for the whole of every matching question, reveal included. The host and player boards showed it too once every clip had been placed.
- Fixed `arrange_in_order` questions announcing the wrong correct answer at the reveal. The shared screen showed the order the question was authored in, and each player's phone showed that player's own submitted order, both labelled "Correct order", while the server had scored against the real answer key. Both surfaces now show the authoritative correct order, and a locked player is no longer told their own answer is the correct one.
- Fixed the Presentation closest-number wall crowning a winner from a partial list of guesses when the guess lookup failed — it could star a player the server had not paid, while the real winner was missing from the board entirely. The wall now says the guesses could not be loaded instead of ranking an incomplete set, and a lookup still in progress no longer reads as "no valid guesses were submitted".
- Stopped the shared screen rebuilding itself on updates that do not change what it is showing. A late player joining, a manual score adjustment, or a host pressing Back mid-question no longer blanks and re-downloads the question or reveal image and restarts the card animations. A manual score adjustment used to do it twice, once on the award and again when the celebration toast expired.
- Fixed tied players being given different finishing positions on different screens. The host leaderboard, the player mini-leaderboard, and the Presentation final-scores pages numbered players by list position, so two players tied for the lead could be exported as joint 1st in the standings CSV but shown as 1st and 2nd on the shared screen — and the second one's phone told them they came second. Every standings surface now shares rank between tied players.
