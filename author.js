...
async function uploadPrivateAudio(file, target = "question") {
  ...
  markChanged();
  const processing = clipped.volumeOverride ? `Rendered clip uploaded at a fixed ${Math.round(clipped.outputGain * 100)}% volume` : "Rendered and loudness-leveled clip uploaded";
...
async function chooseAudioClip(file) {
  ...
  const volumeOverrideEnabled = $("#audio-volume-override-enabled"); const volumeOverrideRange = $("#audio-volume-override-percent"); const volumeOverrideValue = $("#audio-volume-override-value");
  dialog.querySelector(".cropper-head + p").textContent = "Listen to the original, click the waveform to place the red playhead, then set the clip’s in and out points. Press Space to play or pause. Each rendered clip is automatically loudness-leveled to −16 dBFS with a −1 dBFS peak ceiling, unless you override it with a fixed volume below. The original stays local; only the rendered clip uploads.";
  const sourceUrl = URL.createObjectURL(file);
  player.src = sourceUrl;
  let clip = { start: 0, end: source.duration, fadeIn: 0, fadeOut: 0, volumeOverride: false, volumePercent: DEFAULT_MANUAL_AUDIO_VOLUME_PERCENT };
  const sync = () => {
    ...
    clip.volumePercent = clampManualAudioVolumePercent(clip.volumePercent);
    ...
    volumeOverrideEnabled.checked = clip.volumeOverride; volumeOverrideRange.disabled = !clip.volumeOverride; volumeOverrideRange.value = String(clip.volumePercent); volumeOverrideValue.textContent = `${clip.volumePercent}%`;
    $("#audio-clip-preview").textContent = clip.volumeOverride ? `Preview at ${clip.volumePercent}%` : "Preview leveled selection";
    $("#audio-clip-apply").textContent = clip.volumeOverride ? `Render at ${clip.volumePercent}% and upload` : "Render, level and upload";
  ...
  return new Promise((resolve, reject) => {
    const finish = async (apply) => {
      ...
      try {
        const effectiveProcessing = resolveAudioClipProcessing({}, clip.volumeOverride ? clip.volumePercent : null);
        const rendered = await renderAudioClip(source, clip, effectiveProcessing);
        resolve({ blob: audioBufferToWav(rendered.buffer), duration: clip.end - clip.start, normalization: rendered.normalization, outputGain: rendered.outputGain, volumeOverride: clip.volumeOverride });
    ...
    audioClipperSession = {
      ...
      preview: async () => {
        const effectiveProcessing = resolveAudioClipProcessing({}, clip.volumeOverride ? clip.volumePercent : null);
        const rendered = await renderAudioClip(source, clip, effectiveProcessing);
        summary.textContent = clip.volumeOverride ? `Fixed volume override: ${clip.volumePercent}% (${(20 * Math.log10(rendered.outputGain)).toFixed(1)} dB). Automatic loudness leveling bypassed.` : formatNormalization(rendered.normalization);
...
$("#audio-volume-override-enabled").addEventListener("change", (event) => audioClipperSession?.update("volumeOverride", event.target.checked));
$("#audio-volume-override-percent").addEventListener("input", (event) => audioClipperSession?.update("volumePercent", event.target.value));
